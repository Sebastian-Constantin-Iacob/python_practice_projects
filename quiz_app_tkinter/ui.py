...
class QuizInterface:
    ...
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.true_button = Button(image=tru_image, highlightthickness=0, command=self.true_pressed)
    ...
    def get_next_question(self):
        ...
        if self.quiz.still_has_questions():
            ...
            self.score_label.config(text=f"Score: {self.quiz.score}")
            self.canvas.itemconfig(self.question_text, text=f"You have reached the end of the quiz! Score: "
                                                            f"{self.quiz.score}")
